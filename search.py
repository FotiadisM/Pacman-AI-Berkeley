...
def generalSearchAlgorithm(problem, dataStracture):
...
def uniformCostSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    costCalculator = lambda path: problem.getCostOfActions([x[1] for x in path][1:])
    dataStracture = util.PriorityQueueWithFunction(costCalculator)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    "*** YOUR CODE HERE ***"
    costCalculator = lambda path: problem.getCostOfActions([x[1] for x in path][1:]) + heuristic(path[-1][0], problem)
    dataStracture = util.PriorityQueueWithFunction(costCalculator)
